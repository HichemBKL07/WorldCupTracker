...
try {
    ...
    if (isset($_POST['id_nation']) && !isset($_POST['update'])) {
        ...
        if ($nation = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            echo "Confédération : <input type='text' name='confederation' value='" . htmlspecialchars($nation['confederation'] ?? '') . "' required><br>";
            echo "<input type='hidden' name='id_nation' value='" . $nation['id_nation'] . "'>";
